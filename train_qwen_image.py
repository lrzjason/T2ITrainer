...
def main(args, config_args):
    ...
    def train_process(
        ...
        if len(reference_list) > 0:
            ...
            for ref_latent, ref_index in enumerate(reference_list):
                ...
                ref_image_ids = QwenImagePipeline._prepare_latent_image_ids(
                    ref_latent.shape[0],
                    ref_latent.shape[3] // 2,
                    ref_latent.shape[4] // 2,
                    accelerator.device,
                    weight_dtype,
                )
                ref_image_ids[..., 0] = ref_index + 1
        
        
        latent_image_ids = QwenImagePipeline._prepare_latent_image_ids(
            latents.shape[0],
            latents.shape[3] // 2,
            latents.shape[4] // 2,
            accelerator.device,

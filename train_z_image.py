...
from utils.image_utils_zimage import (
    CachedJsonDataset, 
...
import glob
import shutil
# from utils.image_utils_qwenimage import compute_text_embeddings, replace_non_utf8_characters, create_empty_embedding, crop_image


from torchvision import transforms

from utils.utils import find_image_files_by_regex, find_index_from_right, ToTensorUniversal, get_image_files, sample_reference_timesteps, linear_interpolation

from utils.training_set.select_training_set import get_batch_config, get_training_set
...
from utils.utils import project_timestep_snr, print_end_signal
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            'The transformer modules to be excluded for the training. Please specify the layers in a comma seperated. E.g. - "context_refiner,noise_refiner"'
        ),
    )
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="config_z_image_single.json",
    ...
    #     "--use_kl_loss",
    #     action="store_true",
    #     help="Use kl loss to prevent model collapse",
    # )
    # parser.add_argument(
    #     "--kl_weight",
    #     type=float,
    #     default=0.1,
    #     help=("The kl loss weight"),
    # )
    # enable_traj_refs
    # traj_drop_rate = 0.15
    # t_low = 0.2
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("path of timestep_adapter" ),
    )
    # parser.add_argument(
    #     "--use_distributed",
    #     type=bool,
    #     default=False,
    #     help="Check if distributed training is enabled.",
    # )
    # parser.add_argument(
    #     "--use_torch_compile",
    #     action="store_true",
...
def main(args, config_args):
    training_name = "z_image"
    
    # args.timestep_adapter = r"F:\models\Lora\z_image\z_timesteps_adapter-9-3450.safetensors"
    ...
    # dataset_configs = {
    #     # "caption_key":batch_captions_key,
    #     # "latent_key":"latent",
    ...
    if args.skip_lora_layers is not None and args.skip_lora_layers != "":
        skip_modules = [layer.strip() for layer in args.skip_lora_layers.split(",")]
    else:
        # "context_refiner", "noise_refiner"
        skip_modules = []
    ...
    if args.train_data_dir is not None:
        ...
        root_dir = os.path.dirname(args.train_data_dir)
        ...
        @torch.no_grad()
        def cache_process(image_pairs, recreate_cache_target, recreate_cache_reference, recreate_cache_caption):
            datarows = []
            if len(image_pairs) > 0:
                ...
                if os.path.exists(metadata_path) and not args.recreate_cache:
                    try:
                        with open(metadata_path, "r", encoding='utf-8') as readfile:
                            full_datarows += json.loads(readfile.read(), strict=False)
                    except Exception as e:
                ...
                if recache:
                    ...
                    # image_pairs_dataloader = torch.utils.data.DataLoader(
                    #     image_pairs_dataset,
                    #     batch_sampler=image_pairs_sampler,
                    #     collate_fn=collate_fn,
                    #     num_workers=dataloader_num_workers,
                    # )
                    # image_pairs_dataloader = accelerator.prepare(image_pairs_dataloader)
                    ...
                    # for mapping_key, embedding_object in tqdm(embedding_objects.items()):
                        ...
                        if os.path.exists(json_file) and not recreate_cache:
                            full_datarows.append({
                                "json_path": json_file,
                                "bucket": embedding_object["bucket"],
                            })
                            continue
                        ...
                        for caption_config_key in caption_configs.keys():
                            ...
                            if recreate_cache_caption or not os.path.exists(npz_path):
                                ...
                                del npz_dict, prompt_embeds, prompt_embeds_mask
                ...
                if args.validation_ratio > 0 and not os.path.exists(val_metadata_path):
                    ...
                    datarows = training_datarows.copy()
                    ...
                    # Clear memory
                    del validation_datarows, training_datarows, full_datarows
        ...
        cache_process(image_pairs, recreate_cache_target, recreate_cache_reference, recreate_cache_caption)
        flush()
    ...
    if not (args.model_path is None or args.model_path == ""):
        ...
        transformer = ZImageTransformer2DModel.from_single_file(args.model_path, 
                            # config=config,  
    ...
    def load_adapter(model, adapter_path, adapter_name):
        lora_state_dict = safetensors.torch.load_file(adapter_path, device="cpu")
        # metadata = _load_sft_state_dict_metadata(model_file)
        transformer_state_dict = {
            f'{k.replace("diffusion_model.", "")}': v for k, v in lora_state_dict.items() if k.startswith("diffusion_model.")
        }
        transformer_state_dict = convert_unet_state_dict_to_peft(transformer_state_dict)
        incompatible_keys = set_peft_model_state_dict(model, transformer_state_dict, adapter_name=adapter_name)
    ...
    if args.timestep_adapter is not None:
        transformer.load_lora_adapter(args.timestep_adapter, adapter_name="timestep_adapter")
    flush()
    ...
    # print requires_grad_ true
    # print_params_require_grad(transformer)
    ...
    def load_model_hook(models, input_dir):
        ...
        # lora_state_dict = QwenImageEditPipeline.lora_state_dict(input_dir, use_safetensors=True)
        model_file = os.path.join(input_dir, "pytorch_lora_weights.safetensors")
        # remove the default adapter first
        # transformer_.delete_adapters("default")
        # transformer_.load_lora_adapter(model_file, adapter_name="default")
        
        if args.timestep_adapter is not None:
            transformer_.load_lora_adapter(args.timestep_adapter, adapter_name="timestep_adapter")
    ...
    train_dataset = CachedJsonDataset(datarows, 
                                    vae_config_shift_factor, vae_config_scaling_factor, accelerator.device, weight_dtype,
                                    latent_path_key, latent_key, npz_path_key, prompt_embed_key, prompt_embeds_mask_key,
    ...
    def get_sigmas(timesteps, n_dim=4, dtype=torch.float32):
        """
        修复浮点精度问题：查找最接近的时间步索引
        """
        sigmas = noise_scheduler_copy.sigmas.to(device=accelerator.device, dtype=dtype)
        schedule_timesteps = noise_scheduler_copy.timesteps.to(accelerator.device)
        
        # 向量化查找最接近的索引（高效且准确）
        # 将timesteps转换为浮点以匹配schedule_timesteps的类型
        timesteps_float = timesteps.float()
        
        # 计算所有时间步与调度器时间步的绝对差值
        # [batch_size, 1] - [1, num_train_timesteps] -> 广播为 [batch_size, num_train_timesteps]
        diffs = torch.abs(
            schedule_timesteps.unsqueeze(0) - timesteps_float.unsqueeze(1)
        )
        
        # 找到每行最小差值的索引
        step_indices = torch.argmin(diffs, dim=1)
    ...
    # def get_latent(latent_path, vae_config_shift_factor, vae_config_scaling_factor):
    ...
    def train_process(
        ...
        for batch in batches:
            ...
            caption_config = batch_config["caption_config"]
            batch_caption = batch[batch_captions_key][caption_config]
            flush()
            # tcfm_ref_list = []
            target_list = []
            noised_latent_list = []
            ...
            prompt_embed, prompt_embed_mask = batch_caption[prompt_embed_key], batch_caption[prompt_embeds_mask_key]
            ...
            if prompt_embeds is None:
                ...
                try:
                    ...
                    print("prompt_embeds cat error size:", learning_target.shape(), target_list.shape())
                    pass
        
            if prompt_embeds_mask is None:
                prompt_embeds_mask = prompt_embed_mask
            else:
                try:
                    prompt_embeds_mask = torch.cat((prompt_embeds_mask,prompt_embed_mask), dim=0)
                except:
                    print("prompt_embeds_mask cat error size:", learning_target.shape, target_list.shape)
                    pass
        ...
        noise = torch.randn_like(learning_target).to(accelerator.device)
        ...
        # packed_noisy_latents = ZImagePipeline.prepare_latents(
        ...
        #     latents=noisy_model_input,
        # )
        packed_noisy_latents = noisy_model_input.to(accelerator.device)
        # img_shapes = [
        ...
        if len(reference_list) > 0:
            ...
            for ref_latent in reference_list:
                ...
                # pack noisy latents
                # packed_ref_latent = ZImagePipeline.prepare_latents(
                ...
                #     latents=ref_latent,
                # )
                packed_ref_latent = ref_latent.to(accelerator.device)
        ...
        with accelerator.autocast():
            ...
            #         transformer.set_adapter(False)  # 适用于某些PEFT实现
                
            #     teacher_pred = transformer(
            #         model_input,
            #         timesteps / 1000,
            #         prompt_embeds
            #     )[0]
            ...
            #     # 恢复与model_pred相同的后处理
            #     teacher_pred = torch.stack([t.float() for t in teacher_pred], dim=0)
        ...
        # if args.use_kl_loss:
        #     kl_loss = 0.5 * F.mse_loss(
        #         model_pred.float(), 
        #         teacher_pred.detach().float(),  # 确保教师预测无梯度
        #         reduction='mean'
        #     )

        #     # ========== 合并损失 ==========
        #     # 添加kl_weight超参数控制强度 (例如0.1)
        #     total_loss = loss + args.kl_weight * kl_loss
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer):
                batch_config = get_batch_config(batch_configs)
                loss = train_process(
                    batch,

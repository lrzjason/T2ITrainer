...
def get_caption_embedding(npz_path, prompt_embed_key, prompt_embeds_mask_key, device, weight_dtype):
                ...
                for _, configs in group_configs.items():
                    if isinstance(configs, list):
                        ...
                        if self.npz_path_key in configs:
                            item = configs
                            item[self.prompt_embed_key], item[self.prompt_embeds_mask_key] = get_caption_embedding(item[self.npz_path_key], self.prompt_embed_key, self.prompt_embeds_mask_key, self.device, self.weight_dtype)
...
def compute_text_embeddings(text_encoders, tokenizers, prompt, device, image=None, processor=None, instruction=None, drop_index=None):
    with torch.no_grad():
        prompt_embeds, prompt_embeds_mask = encode_prompt(text_encoders, 
                        tokenizers, prompt, device=device
...
def get_empty_embedding(cache_path="cache/empty_embedding.npzimage"):
    if os.path.exists(cache_path):
        return torch.load(cache_path, weights_only=True)
    else:
        raise FileNotFoundError(f"{cache_path} not found")
def create_empty_embedding(tokenizers,text_encoders,cache_path="cache/empty_embedding.npzimage",recreate=False):
    if recreate:
...
def encode_prompt_with_qwenvl(
    ...
    prompt_embeds_mask = text_inputs.attention_mask.to(device).bool()

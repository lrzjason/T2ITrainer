...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="config_longcat_edit_local.json",
...
def main(args, config_args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            # save all
            LongCatImageEditPipeline.save_lora_weights(
                output_dir,
    ...
    def train_process(
        ...
        packed_noisy_latents = LongCatImageEditPipeline._pack_latents(
            noisy_model_input,
        ...
        if len(reference_list) > 0:
            ...
            for ref_latent in reference_list:
                # pack noisy latents
                ref_latent = torch.stack([ref_latent], dim=0)
                packed_ref_latent = LongCatImageEditPipeline._pack_latents(
                    ref_latent,
                ...
                if ref_height == 0:
                    ref_height = ref_latent.shape[2]
                ref_width += ref_latent.shape[3] 
                # print("packed_ref_latent.shape",packed_ref_latent.shape)
                if packed_ref_latents is None:
                    packed_ref_latents = packed_ref_latent
                else:
                    # due to one image, we need to cat on width
                    packed_ref_latents = torch.cat([packed_ref_latents, packed_ref_latent], dim=-1)
        
            ref_latents_ids = prepare_pos_ids(modality_id=2,
                                           type='image',
                                           start=(prompt_embeds.shape[1],
                                                  prompt_embeds.shape[1]),
                                           height=ref_height//2,
                                           width=ref_width//2).to(accelerator.device, dtype=torch.float32)
        ...
        if packed_ref_latents is not None:
            ...
            model_input = torch.cat([model_input, packed_ref_latents], dim=1)
            latent_image_ids = torch.cat([latent_image_ids, ref_latents_ids], dim=0)
        ...
        model_pred = LongCatImageEditPipeline._unpack_latents(
            model_pred,
        ...
        # remove weighting refer to author
        loss = torch.mean(
